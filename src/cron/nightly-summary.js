// Optional nightly summary: digest of day (completed projects, active reminders)
...
export async function handleNightlySummary(config, env, ctx) {
  try {
    ...
    try {
      ...
      const AIProviderManager = (await import("../lib/ai.js")).AIProviderManager;
      ...
      // Adaptive personality learning: identify stable communication preferences from
      // recent conversations. Gated by the personality_optimization_enabled setting
      // and bounded to a small conversation window to keep cost low.
      const { learnAdaptiveProfile } = await import("../lib/adaptive.js");
      const adaptiveResult = await learnAdaptiveProfile(config, env, aiManager2);
      if (adaptiveResult.learned > 0) {
        await log(env.DB, "info", "adaptive_profile_learned_daily", { learned: adaptiveResult.learned });
      }
    } catch (summaryError) {
