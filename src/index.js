...
import { handleQuestionPage } from "./admin/question.js";
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      // Daily Plan questionnaire / plan view (public, token-authenticated).
      // Bound to a per-plan random access token; never requires admin login.
      const questionMatch = pathname.match(/^\/question\/([a-f0-9]+)\/?$/);
      if (questionMatch) {
        return await handleQuestionPage(request, env, config, questionMatch[1]);
      }
