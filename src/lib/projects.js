...
export class ProjectManager {
  ...
  async updateProject(projectId, updates) {
    try {
      ...
      if (changes === 0) return { success: false, notFound: true, error: "Project not found" };
  ...
  // Single-owner bot; projects carry no owner column. Returns every match so the
  // caller can disambiguate.
  async findProjects(reference, limit = 20) {
    if (!reference) return [];
    const haystack = toDigits(String(reference)).toLowerCase();
    const rows = (await this.db.prepare(
      "SELECT * FROM projects WHERE status IN ('active', 'paused') ORDER BY updated_at DESC LIMIT ?"
    ).bind(limit).all()).results || [];
    return rows.filter((p) => toDigits(`${p.name || ""} ${p.client || ""} ${p.notes || ""}`).toLowerCase().includes(haystack));
  }
  ...
  async deleteProject(id) {
    ...
    const changes = result.meta?.changes || 0;
    if (changes === 0) return { success: false, notFound: true, error: "Project not found" };
    await this.logger.info(this.db, "projects", "deleted", { id });
    return { success: true };
  }
}