...
export class SessionManager {
  ...
  // Single source of truth for the effective Judge routing state, shared by the
  // message pipeline, /status, and the /judge_on | /judge_off commands.
  // globalEnabled: admin-controlled judge_routing_enabled setting.
  // chatOverride: per-chat /judge_on|/judge_off override stored in state_json —
  //   null = no override, false = chat enabled (/judge_on), true = chat disabled (/judge_off).
  // enabled: whether Judge actually runs for this session. Global OFF always wins;
  //   a chat /judge_on cannot bypass a globally disabled Judge.
  async getEffectiveJudgeState(sessionId, globalJudgeEnabledOverride = null) {
    let globalEnabled = false;
    if (globalJudgeEnabledOverride !== null) {
      globalEnabled = globalJudgeEnabledOverride;
    } else {
      try {
        const setting = await this.db.prepare("SELECT value FROM settings WHERE key = 'judge_routing_enabled'").first();
    ...
    try {
      ...
      if (state && state.judge_disabled !== undefined) {
        chatOverride = state.judge_disabled === true;
      }
    } catch (e) {}
    return {
      globalEnabled,
      chatOverride,
      enabled: globalEnabled && chatOverride !== true
    };
  }

  async setJudgeOverride(sessionId, disabled) {
    await this.updateSessionState(sessionId, { judge_disabled: disabled === true });
  }
