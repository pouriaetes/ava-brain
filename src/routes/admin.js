...
import { handleCapabilitiesPage } from "../admin/capabilities.js";
import { handleMemoryPage } from "../admin/memory.js";
import { handleRemindersPage } from "../admin/reminders.js";
import { handleDailyPlanPage } from "../admin/daily-plan.js";
import { handleLogsPage } from "../admin/logs.js";
import { handleTracesPage } from "../admin/traces.js";
...
export async function handleAdmin(request, env, config) {
  ...
  switch (pathname) {
    ...
    case "/admin/ava_brain/capabilities":
      return await handleCapabilitiesPage(request, env, config);
    ...
    case "/admin/ava_brain/daily_plan":
      return await handleDailyPlanPage(request, env, config);
    case "/admin/ava_brain/logs":
      return await handleLogsPage(request, env, config);
    case "/admin/ava_brain/traces":
      return await handleTracesPage(request, env, config);
    default:
...
async function handleLogin(request, env, config) {
  ...
  if (request.method === "POST") {
    ...
    const db = env.DB;
    let user = await verifyAndGetUser(db, username, password);
    if (!user) {
      // Self-healing (seed/reseed the admin row) only when the normal login
      // failed, so an already-configured install does not run these on every POST.
      await ensureSeededPassword(db);
      await checkSingleUser(db);
      user = await verifyAndGetUser(db, username, password);
    }
    if (!user) {
